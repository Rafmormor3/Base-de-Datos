...
SELECT COUNT(EMPLE.EMP_NO) AS NUMERO_EMPLE 
FROM EMPLE
...
SELECT AVG(EMPLE.SALARIO) AS SALARIO_MEDIO  
FROM EMPLE 
GROUP BY EMPLE.DEPT_NO  ;
...
SELECT COUNT(EMPLE.EMP_NO) AS NUMERO_EMPLEADOS
FROM EMPLE 
...
SELECT * 
FROM (SELECT HERRAMIENTAS.ESTANTERIA , SUM(HERRAMIENTAS.UNIDADES) AS SUMA_UNIDADES
		FROM HERRAMIENTAS
		GROUP BY HERRAMIENTAS.ESTANTERIA 
		ORDER BY SUMA_UNIDADES DESC)
WHERE ROWNUM=1;
...
FROM MEDICOS
GROUP BY MEDICOS.ESPECIALIDAD 
ORDER BY COUNT(MEDICOS.DNI) DESC LIMIT 1;

SELECT *
FROM (SELECT MEDICOS.ESPECIALIDAD , COUNT(MEDICOS.DNI)
		FROM MEDICOS
		GROUP BY MEDICOS.ESPECIALIDAD 
		ORDER BY COUNT(MEDICOS.DNI) DESC)
WHERE ROWNUM=1;
...
SELECT HOSPITALES.NOMBRE, MAX(NUM_PLAZAS)  
FROM HOSPITALES
GROUP BY NOMBRE 
ORDER BY MAX(NUM_PLAZAS) DESC LIMIT 2;

SELECT *
FROM (SELECT HOSPITALES.NOMBRE  
		FROM HOSPITALES 
		ORDER BY HOSPITALES.NUM_PLAZAS DESC)
WHERE ROWNUM=2;
