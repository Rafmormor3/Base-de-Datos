...
CREATE OR REPLACE PACKAGE BODY CARRERACABALLOS.MORALESGESTIONCARRERAS IS
	...
	BEGIN
		...
		WHEN NO_DATA_FOUND THEN
			CODIGO_PERSON:=CODIGO_PERSON+1;
			INSERT INTO PERSONAS(CODIGO,DNI,NOMBRE) VALUES (CODIGO_PERSON, DNIDUEÑO, DUEÑO);
		
		SELECT COUNT(C.NOMBRE)
		INTO CABALLO
		FROM CABALLOS c 
		WHERE C.NOMBRE LIKE NOMBRECAB;
	
		IF CABALLO>0 THEN
		 RAISE_APPLICATION_ERROR(-20007,'CABALLO YA EXISTE');
		ELSE
			CODIGO_CAB:=CODIGO_CAB+1;
			INSERT INTO CABALLOS(CODCABALLO, NOMBRE, PESO, FECHANACIMIENTO, PROPIETARIO, NACIONALIDAD) 
		 		VALUES(CODIGO_CAB, NOMBRECAB, PESOCAB, FECHANAC, COD, NACION);
		END IF;
		...
		INTO NAC
		FROM CABALLOS c 
		WHERE C.CODCABALLO = CODIGO_CAB;
		
		IF NAC IS NULL THEN
			UPDATE CABALLOS SET NACIONALIDAD = 'ESPAÑOLA' WHERE CODCABALLO = CODIGO_CAB;
		END IF;
...
BEGIN
	MORALESGESTIONCARRERAS.AGREGARCABALLOS('RELAMPAGO', 250, SYSDATE, NULL, '0000000P', 'MANOLIN');
END;
