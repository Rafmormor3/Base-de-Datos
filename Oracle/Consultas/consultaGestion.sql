...
SELECT CODFAC, FECHA, NVL(DTO*2,0) AS DOBLEDESCUENTO FROM FACTURAS WHERE IVA=0;
...
--4. Código de factura y de artículo de las líneas de factura en las que la cantidad 
--solicitada es menor de 5 unidades y además se ha aplicado un descuento del 25% o mayor
SELECT CODFAC, CODART FROM LINEAS_FAC WHERE CANT<5 AND DTO>=25;

--5.Obtener la descripción de los artículos cuyo stock está por debajo del stock 
--mínimo, dado también la cantidad en unidades necesaria para alcanzar dicho mínimo.
SELECT DESCRIP, (STOCK_MIN-STOCK) AS DIFERENCIA_STOCK FROM ARTICULOS WHERE STOCK < STOCK_MIN;
...
SELECT NOMBRE FROM PROVINCIAS WHERE UPPER(NOMBRE) LIKE '_O%';
...
SELECT CODFAC, FECHA FROM FACTURAS WHERE EXTRACT(MONTH FROM FECHA)=6 AND EXTRACT(YEAR FROM FECHA)=2004;

--18.Código y fecha de las facturas que se han realizado durante el mes de junio del año 2004 para aquellos clientes cuyo código se 
--encuentra entre 100 y 250.
SELECT CODFAC, FECHA FROM FACTURAS WHERE EXTRACT(MONTH FROM FECHA)=6 AND EXTRACT(YEAR FROM FECHA)=2004 AND CODCLI BETWEEN 100 AND 250;
...
SELECT NOMBRE FROM PROVINCIAS WHERE UPPER(NOMBRE) LIKE '%S';
...
--se hará un descuento del 10%. Mostrar un listado de los artículos que van a entrar en la promoción, con su código de artículo, 
--nombre del articulo, precio actual y su precio en la promoción.
SELECT CODART, DESCRIP, PRECIO, PRECIO*0.9 AS PRECIODESCONTADO FROM ARTICULOS WHERE PRECIO>6000 AND STOCK*PRECIO>6000;
